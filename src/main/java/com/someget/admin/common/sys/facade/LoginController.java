...
import com.someget.admin.common.util.VerifyCodeUtil;
...
public class LoginController extends BaseController {
	...
	public String login(HttpServletRequest request) {
		...
		log.info("是否记住登录--->{}<-----是否有权限登录----->{}<----", s.isRemembered(), s.isAuthenticated());
	...
	@ResponseBody
	public RestResponse loginMain(HttpServletRequest request){//不知道springmvc传参怎么用吗???
		Subject user = null;
		String error = null;
		...
		User baseUser = userService.findBaseUserByLoginName(username);
		...
		if(StringUtils.isBlank(error)){
			//登录后将serviceId放在redis, 需要在登出或者登录失效时删除.
			//redisTemplate.opsForValue().set("",user.getPrincipal());
	...
	public String showView(Model model){
		// 注意,这里可以下发不同的值, 决定登陆后跳转页面是哪里
		model.addAttribute("gotoType", 1);
		return "index";
	}
	...
	public void genCaptcha(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		//将验证码放到HttpSession里面
		request.getSession().setAttribute(SecurityConstants.VALIDATE_CODE, verifyCode);
	...
	public String main(Model model){
		...
		Set<com.someget.admin.common.sys.dal.entity.Menu> menus = user.getMenus();
		java.util.List<com.someget.admin.common.sys.dal.entity.Menu> showMenus = Lists.newArrayList();
		if(menus != null && menus.size()>0){
			for (com.someget.admin.common.sys.dal.entity.Menu menu : menus){
 ...
 * @date 2022-03-25 20:03
 */
class MenuComparator implements Comparator<com.someget.admin.common.sys.dal.entity.Menu>{

	@Override
	public int compare(com.someget.admin.common.sys.dal.entity.Menu o1, Menu o2) {
